...
- Animation and transition timings (chat message reveal, code block typewriter effect on the homepage, runtime-flow diagram animations) are not in scope.
...
- The "computer use" demo cards on certain pages display animated Claude controlling a browser — the static screenshot doesn't fully capture the animation chrome.
...
1. 产品标识：深色方形图标容器 + `ShieldCheck` 或 `Terminal` 图标。
...
2. 左侧索引：问题索引、Pi 时间线、页面锚点。
3. 右侧主内容：问题清单、技术走查、Pi Runtime 追溯、原始材料。
