...
- 不同仓库绑定不同 Bubblewrap runtime workspace，同仓库复用同一个 bare repo 缓存。
- 同仓库并发 review 共享仓库缓存，但使用独立 `git worktree`、独立 Pi 进程和独立 `ReviewSandboxSession`。
- 应用不挂载 Docker socket，不直接管理宿主机容器，Pi 进程由 Bubblewrap 做文件系统隔离。
...
- `GET /api/health`：检查 PostgreSQL、Bubblewrap 和 Pi Runtime 配置。
...
1. 创建或连接 `RepositorySandboxBinding` 对应的 Bubblewrap runtime workspace。
2. 创建 `ReviewSandboxSession`，记录当前 review 的 worktree、Pi command 和运行状态。
3. 在 runtime workspace 内准备 bare repo，并 fetch 最新 refs。
4. 为本次 review 创建独立 `git worktree`。
5. 写入 `/tmp/pi-review-input.json` 和 Pi prompt 文件。
6. 通过 Bubblewrap 在 worktree 中执行 `/opt/pi/bin/pi -p --no-context-files --no-session`。
7. 解析 Pi 严格 JSON 输出。
8. 校验 finding 是否命中本次 diff。
9. 完成后清理 worktree；若没有其他 running session，则把 binding 标记为 paused。
...
Bubblewrap 和 Git 安装在 app 镜像内。`docker-compose.yml` 为 app 容器挂载
`pi_runtime_workspace`，保存仓库 bare repo 缓存、review worktree 和运行时临时文件。

Pi 安装在 `/opt/pi`。Bubblewrap 运行 Pi 时只读挂载 Pi 安装目录、当前仓库
bare repo 和当前 review worktree。app 不需要 Docker socket。

关键环境变量：

```text
BUBBLEWRAP_BIN
BUBBLEWRAP_WORKSPACE_ROOT
PI_HOST_PATH
...
```text
repo A -> runtime workspace A
repo B -> runtime workspace B
```
...
- 没有 running session 时把 binding 标记为 `paused`，保留仓库缓存。
...
- Bubblewrap 不可用或路径检查失败：`ReviewSandboxSession` failed，binding 标记 error。
...
- Bubblewrap 配置读取和 provider 快速失败。
