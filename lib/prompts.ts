...
请基于 Code Graph、调用链上下文和 diff，输出一个合法 JSON 对象：
{
  ...
  "shouldUseAdditionalAgents": false,
  "additionalAgentReason": "是否需要调用其他专门审查 Agent 的原因"
}

约束：
- 不要输出 Markdown 或代码块。
- requestedTools 只能从 get_code_graph_status、get_memory_snapshot、search_memory_facts、get_changed_files、get_file_context、get_call_graph_neighbors、get_related_review_history、get_architecture_summary、rebuild_code_graph 中选择。
...
- 默认由当前 Agent 完成审查；只有变更高风险、跨多个关键模块、涉及安全/数据一致性/复杂架构边界，或需要另一个专门 Prompt 复核时，shouldUseAdditionalAgents 才能为 true。`

/** Agent Loop 结构化审查提示词 */
export const REVIEW_AGENT_REVIEW_SYSTEM_PROMPT = `你是一名代码审查 Agent。
请结合 Code Graph、调用链上下文、历史审查和本轮计划，只输出可定位、可修复的问题，并用 confidence 标注把握程度。
