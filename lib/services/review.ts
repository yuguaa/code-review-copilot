...
export class ReviewService {
  ...
  async performReview(reviewLogId: string) {
    ...
    try {
      ...
      console.log(
        `📁 [ReviewService] Files to review: ${relevantDiffs.length}`,
      );
      ...
      for (const diff of relevantDiffs) {
        ...
        console.log(
          `   - customPrompt: ${repository.customPrompt ? "已设置 (" + repository.customPrompt.substring(0, 50) + "...)" : "未设置"}`,
        );
        console.log(
          `   - customPromptMode: ${(repository as any).customPromptMode || "未设置 (默认 extend)"}`,
        );

        if (repository.customPrompt) {
          const promptMode = (repository as any).customPromptMode || "extend";
          if (promptMode === "replace") {
            // 替换模式：使用自定义提示词 + 基础输出格式（确保解析器可用）
            systemPrompt = repository.customPrompt + OUTPUT_FORMAT;
            console.log(
              `📝 [ReviewService] Using REPLACE mode - custom prompt + output format`,
            );
            console.log(
              `📝 [ReviewService] System prompt: ${systemPrompt.substring(0, 100)}...`,
            );
          } else {
            // 扩展模式：内置 + 自定义
            systemPrompt = `${SYSTEM_PROMPT}\n\n【仓库自定义要求】\n${repository.customPrompt}`;
            console.log(
              `📝 [ReviewService] Using EXTEND mode - built-in + custom prompt`,
            );
          }
        }
        ...
        console.log(
          `💬 [ReviewService] Found ${comments.length} comments in ${filePath}`,
        );
      ...
      // 统计问题数量
      const criticalIssues = totalComments.filter(
        (c) => c.severity === "critical",
      ).length;
      const normalIssues = totalComments.filter(
        (c) => c.severity === "normal",
      ).length;
      const suggestions = totalComments.filter(
  ...
  async postCommentsToGitLab(reviewLogId: string, gitlabService: any) {
    ...
    if (reviewLog.mergeRequestIid === 0) {
      console.log(
        `📝 [ReviewService] Push event - posting comments to commit: ${reviewLog.commitSha}`,
      );
    ...
    for (const comment of reviewLog.comments) {
      try {
        ...
        console.log(
          `✅ Posted inline comment: ${comment.filePath}:${comment.lineNumber}`,
        );
      } catch (error) {
        // 行内评论失败，尝试发布普通评论
        console.log(
          `⚠️ Inline comment failed for ${comment.filePath}:${comment.lineNumber}, trying general comment...`,
        );
        ...
        try {
          ...
          console.log(
            `✅ Posted general comment for: ${comment.filePath}:${comment.lineNumber}`,
          );
        } catch (fallbackError) {
          console.error(
            `❌ Failed to post comment for ${comment.filePath}:${comment.lineNumber}`,
          );
        }
      }
    }
  ...
  async postCommentsToCommit(reviewLog: any, gitlabService: any) {
    ...
    console.log(
      `📤 [ReviewService] Posting ${comments.length} comments to commit ${reviewLog.commitSha}`,
    );
    ...
    for (const comment of comments) {
      try {
        ...
        console.log(
          `✅ Posted inline comment to commit: ${comment.filePath}:${comment.lineNumber}`,
        );
      } catch (error) {
        // 行内评论失败，尝试普通评论
        console.log(
          `⚠️ Inline commit comment failed, trying general comment...`,
        );
        ...
        try {
          ...
          console.error(
            `❌ Failed to post comment to commit: ${comment.filePath}:${comment.lineNumber}`,
          );
        }
      }
    }
