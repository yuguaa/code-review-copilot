 ...
 */
export function buildInstructions(repo: SessionWithRepository['repository'], capabilities?: Pick<CapabilityState, 'skills'>): string {
...
- 全程用简体中文。`;
  ...
  const skillInstructions = renderSkillInstructions(capabilities?.skills ?? []);
...
export async function createReviewStream(opts: { session: SessionWithRepository; messages: UIMessage[] }) {
  ...
  const capabilities = repo ? await resolveRepositoryCapabilities(repo.id) : undefined;
  if (capabilities) ctx.enabledTools = capabilities.tools;
  const tools = { ...buildTools(ctx), ...buildDelegateTools(ctx, model) };

  return streamText({
    model,
    system: buildInstructions(repo, capabilities),
