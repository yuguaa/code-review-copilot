...
字段约束：
- 所有字符串值必须符合 JSON 字符串规则：字符串内部的双引号必须转义为 \\"，换行必须写成 \\n。
...
- 不要输出 Markdown 或代码块。
- 所有字符串值必须符合 JSON 字符串规则：字符串内部的双引号必须转义为 \\"，换行必须写成 \\n。
- requestedTools 只能从 get_code_graph_status、get_memory_snapshot、get_changed_files、get_file_context、get_call_graph_neighbors、get_related_review_history、get_architecture_summary、rebuild_code_graph 中选择。
- 辅助 Agent 由系统在主 Agent 校验出需复核问题后统一调用，规划阶段不要请求或点名辅助 Agent。
