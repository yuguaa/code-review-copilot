...
export function buildInstructions(
  ...
  const tools = [
    ...
    '- record_evidence：把本轮已核验证据记录到运行期 CodeMem，供可选的 verify 增强复核',
...
- 本轮采用 DeepCode 风格链路：先遵循审查蓝图取证，再把已确认材料写入运行期 CodeMem，最后尝试交给 verify 增强复核。运行期 CodeMem 不是长期记忆。
...
- 不要写项目记忆；系统只把 verify 增强成功后的结论沉淀为后续审查依据。
- 你的输出可能进入只读 verify 增强复核；无论增强是否可用，主审查总评都必须完整、可独立使用。
- 全程用简体中文。`;
