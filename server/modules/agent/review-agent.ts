...
- 如果项目记忆包含「用户反馈沉淀」，把用户认可的审查发现作为后续重点复核模式；把用户否定的审查发现作为反例，除非本次变更有新的明确证据，否则不要机械复读。
...
- 自行判断是否需要委派 subagents：涉及安全/架构/性能的复杂变更才调用对应 delegate 工具；简单、低风险、证据已充分的变更不要为了形式委派。
...
export async function createReviewStream(opts: { session: SessionWithRepository; messages: UIMessage[] }) {
  ...
  const delegateModels = modelConfigs.delegates.map(resolveModel);
  ...
  const tools = { ...buildTools(ctx, { publish: false, memoryWrite: false }), ...buildDelegateTools(ctx, delegateModels) };
  ...
  return { stream, ctx, verifierModel: resolveModel(modelConfigs.verifier), verifierConfig: modelConfigs.verifier };
}
