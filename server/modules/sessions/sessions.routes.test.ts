...
describe('session review command route', () => {
  it('delegates slash review commands to the session service', () => {
    ...
    expect(routeSource).toContain("result.kind === 'not-running'");
    ...
    expect(serviceSource).toContain('stopRunningReviewSession(sessionId)');
    expect(serviceSource).toContain('markReviewSessionStopped(sessionId)');
    ...
    expect(lifecycleSource).toContain('STOPPED_REVIEW_ERROR');
  });
});
