...
export function buildDashboardPayload(sessions: SessionRow[], counts: DashboardCounts, now = new Date()) {
  ...
  return {
    ...
    statusDistribution: ['completed', 'failed', 'running']
      .map((status) => ({ status, count: byStatus.get(status) ?? 0, percent: percent(byStatus.get(status) ?? 0, total) })),
...
dashboardRoutes.get('/', async (c) => {
  ...
  const [sessions, repositoryCount, activeRepositoryCount, modelCount, accountCount, toolCount, skillCount] = await prisma.$transaction([
  ...
  return c.json(
    buildDashboardPayload(sessions, {
      repositoryCount,
