...
function EventPart({ part }: { part: Record<string, unknown> }) {
  ...
  return (
    <div className="my-1.5 overflow-hidden rounded-xl bg-white text-xs shadow-sm ring-1 ring-slate-200/80">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center gap-2 px-3 py-2 text-left text-slate-600 transition-[background-color] hover:bg-indigo-50/60"
      >
    ...
    <div className={cn('flex gap-3 py-3', isUser ? 'justify-end pl-16' : 'justify-start pr-16')}>
      <div
        className={cn(
          ...
          isUser
            ? 'order-2 bg-indigo-600 text-white shadow-indigo-600/25'
            : 'bg-white text-indigo-600 ring-1 ring-slate-200/80',
        )}
      >
      ...
      <div className={cn('flex min-w-0 max-w-[72%] flex-col space-y-1', isUser ? 'items-end text-right' : 'items-start')}>
        {message.parts.map((part, i) => {
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isUser
                    ? 'rounded-2xl rounded-tr-md bg-indigo-600 text-white shadow-indigo-600/20'
          ...
          if (part.type === 'reasoning') {
            return (
              <div key={i} className="rounded-lg border-l-2 border-indigo-200 bg-slate-50 px-3 py-2 text-xs italic text-slate-500">
                {part.text}
              </div>
            );
          }
